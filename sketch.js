...
function draw() {
  ...
  if(tower.y > 400){
    ...
    drawSprites();
    createEdgeSprites();
    tower.velocityY =(1 + frameCount/200);
    ...
    if(ghost.isTouching(invisibleBlockGroup)||ghost.y >550)
    {
      ghost.destroy();
...
if (frameCount % 240===0)
{
 ...
 // door.scale=0.5;
  door.velocityY=(1 +  frameCount/200);
  door.lifetime=400;
  doorsGroup.add(door);
  ...
  //climber.scale=0.5;
  climber.velocityY=(1 +  frameCount/200);
  climber.lifetime=400;
  climbersGroup.add(climber);
  climber.x=door.x;

  invisibleBlock=createSprite(200,120);
  invisibleBlock.velocityY=(1 +  frameCount/200);
